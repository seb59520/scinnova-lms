 ...
 * 
 * Usage:
 *   node import-course-direct.cjs <fichier-json> [--update <course-id>]
 * 
 * Exemple:
 *   node import-course-direct.cjs architecture-client-serveur-web.json
 *   node import-course-direct.cjs architecture-client-serveur-web.json --update abc123-def456
 */

const fs = require('fs')
const path = require('path')

// Fonction pour charger le .env manuellement
...
if (!envLoaded) {
  // Essayer dotenv si disponible
  try {
    require('dotenv').config()
  } catch (e) {
    // dotenv n'est pas installé, on continue avec les variables système
  }
}
...
if (!SUPABASE_URL) {
  ...
  console.error('   Assurez-vous d\'avoir VITE_SUPABASE_URL dans votre .env')
  process.exit(1)
}
...
if (!SUPABASE_KEY) {
  ...
  console.error('   Assurez-vous d\'avoir VITE_SUPABASE_ANON_KEY ou SUPABASE_SERVICE_ROLE_KEY dans votre .env')
  console.error('   Note: SUPABASE_SERVICE_ROLE_KEY est recommandé pour bypasser RLS')
  process.exit(1)
}

if (SUPABASE_SERVICE_KEY) {
  console.log('✅ Utilisation de SUPABASE_SERVICE_ROLE_KEY (bypass RLS)')
} else {
  console.log('⚠️  Utilisation de VITE_SUPABASE_ANON_KEY (soumise à RLS)')
  console.log('   Si vous rencontrez des erreurs RLS, ajoutez SUPABASE_SERVICE_ROLE_KEY dans votre .env')
...
async function importCourse(jsonFilePath, updateCourseId = null) {
  ...
  try {
    ...
    if (updateCourseId) {
      ...
      // Si on utilise SERVICE_ROLE_KEY, on peut récupérer le premier admin ou utiliser USER_ID
      let userId = process.env.USER_ID
      
      if (!userId && SUPABASE_SERVICE_KEY) {
        // Essayer de récupérer le premier utilisateur admin
        try {
          const admins = await supabaseRequest('/rest/v1/profiles?role=eq.admin&select=id&limit=1')
          if (Array.isArray(admins) && admins.length > 0) {
            userId = admins[0].id
            console.log(`   Utilisation de l'utilisateur admin: ${userId}`)
          }
        } catch (e) {
          console.warn('   ⚠️  Impossible de récupérer un utilisateur admin, utilisation d\'un UUID par défaut')
        }
      }
      
      if (!userId) {
        userId = '00000000-0000-0000-0000-000000000000'
        console.warn('   ⚠️  Utilisation d\'un UUID par défaut pour created_by')
        console.warn('   💡 Définissez USER_ID dans votre .env pour utiliser votre ID utilisateur')
      }

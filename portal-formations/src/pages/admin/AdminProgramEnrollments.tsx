...
import { Profile, ProgramEnrollment, Program } from '../../types/database'
import { Users, Search, Plus, X } from 'lucide-react'

interface EnrollmentWithProfile extends ProgramEnrollment {
  profiles: Profile
}

export function AdminProgramEnrollments() {
  ...
  const handleAddEnrollments = async () => {
    ...
    try {
      ...
      if (newUserIds.length === 0) {
        setError('Toutes les personnes sélectionnées ont déjà accès au programme.')
        setSaving(false)
        return
      }
  ...
  const handleRemoveEnrollment = async (enrollmentId: string) => {
    if (!confirm('Êtes-vous sûr de vouloir retirer cette personne du programme ?')) return
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900">
                  Gestion des accès au programme
                </h1>
                {program && (
          ...
          <div className="bg-white shadow overflow-hidden sm:rounded-md">
            <div className="px-6 py-4 border-b border-gray-200">
              <h2 className="text-lg font-medium text-gray-900">
                Personnes ayant accès ({filteredEnrollments.length})
              </h2>
            </div>
            ...
            {filteredEnrollments.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="mt-4 text-gray-500">
                  {searchTerm ? 'Aucune personne ne correspond à votre recherche.' : 'Aucune personne n\'a accès pour le moment.'}
                </p>
              </div>
            ) : (
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-gray-500 mt-1">
                          Accès accordé le {new Date(enrollment.enrolled_at).toLocaleDateString('fr-FR', {
                            day: 'numeric',
                            month: 'long',
                            year: 'numeric',
                            hour: '2-digit',
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
          <div className="relative top-20 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-1/2 shadow-lg rounded-md bg-white">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-medium text-gray-900">
                Ajouter des personnes au programme
              </h3>
              <button
                onClick={() => {
                  setShowAddModal(false)
            ...
            <div className="max-h-96 overflow-y-auto mb-4 border border-gray-200 rounded-md">
              {filteredAvailableUsers.length === 0 ? (
                <div className="text-center py-8 text-gray-500">
                  {modalSearchTerm ? 'Aucune personne ne correspond à votre recherche.' : 'Toutes les personnes ont déjà accès.'}
                </div>
              ) : (
                <ul className="divide-y divide-gray-200">

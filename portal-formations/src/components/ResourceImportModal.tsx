...
import type { ResourceType } from '../types/resourcesWidget'
...
export function ResourceImportModal({
  courseId,
  moduleId,
  itemId,
  isOpen,
  ...
  const parentType = courseId ? 'course' : moduleId ? 'module' : itemId ? 'item' : null

  const downloadTemplate = (format: 'csv' | 'json') => {
    if (format === 'csv') {
      const csvContent = `title,description,resource_type,external_url,is_required,is_visible
...
Documentation officielle,Documentation officielle Python,url,https://www.python.org/doc/,true,true`
      ...
      link.download = 'template-ressources.csv'
      link.click()
    } else {
      const jsonContent = {
        resources: [
      ...
      const link = document.createElement('a')
      link.href = URL.createObjectURL(blob)
      link.download = 'template-ressources.json'
      link.click()
    }
  }
  ...
  const parseCSV = (csvText: string): ImportResourceRow[] => {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const validTypes: ResourceType[] = ['file', 'url', 'video', 'document', 'code', 'data']
      if (!validTypes.includes(row.resource_type as ResourceType)) {
        throw new Error(`Ligne ${i + 1}: type de ressource invalide. Types valides: ${validTypes.join(', ')}`)
      }

      // Pour les types url et video, external_url est requis
  ...
  const parseJSON = (jsonText: string): ImportResourceRow[] => {
    ...
    data.resources.forEach((resource: any, index: number) => {
      ...
      resources.push({
        ...
        is_required: resource.is_required === true || resource.is_required === 'true',
        is_visible: resource.is_visible !== false && resource.is_visible !== 'false'
      })
    })
  ...
  const parseURLs = (urlsText: string): ImportResourceRow[] => {
    ...
    lines.forEach((line, index) => {
      ...
      // Détecter si c'est une URL valide
      try {
        new URL(trimmed)
      } catch {
  ...
  const handleImport = async () => {
    if (!parentId || parentId === 'new' || !profile) {
      setError('Impossible d\'importer: parent invalide ou profil non chargé')
      return
    }

    const allowedRoles = ['admin', 'trainer', 'instructor']
    if (!allowedRoles.includes(profile.role)) {
      setError('Vous n\'avez pas les permissions pour importer des ressources')
      return
    }
    ...
    try {
      ...
      // Parser selon la méthode choisie
      ...
      // Récupérer l'ordre actuel pour continuer la numérotation
      let query = supabase.from('resources').select('order_index')
      if (courseId) query = query.eq('course_id', courseId)
      else if (moduleId) query = query.eq('module_id', moduleId)
      else if (itemId) query = query.eq('item_id', itemId)

      const { data: existingResources } = await query.order('order_index', { ascending: false }).limit(1)
      let nextOrderIndex = existingResources && existingResources.length > 0 
      ...
      for (const resource of resourcesToImport) {
        try {
          const resourceData: any = {
            ...
            is_visible: resource.is_visible !== false,
